...
(deftest conv-varchar
  (is (= [:input.form-control {:id "name" :required "required" :maxlength 40}] (tth/dt->hiccup varchar-c)))
  (is (thrown? AssertionError (tth/dt->hiccup varchar-invalid-c))))

(deftest conv-char
  (is (= [:input.form-control {:id "name" :required "required" :maxlength 40}] (tth/dt->hiccup char-c))))

(deftest conv-boolean
  (is (= [:input.form-control {:id "male" :checked "checked"}] (tth/dt->hiccup bool-c)))
  (is (= [:input.form-control {:id "male"}] (tth/dt->hiccup (assoc bool-c 3 false)))))